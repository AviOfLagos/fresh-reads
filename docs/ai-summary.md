...
> Status: **Live.** Summaries are generated by `summarizeArticle`
> (`src/server/summarize.ts`) calling the Lovable AI Gateway with
> `google/gemini-2.5-flash` and structured tool-calling. Results are cached
> per `articleId` in `localStorage` (`newsroom.summaries.v1`, last 200).

## What it does

- "AI Brief" expandable panel on every article page.
- TL;DR + up to 5 bullets + "Why it matters" + estimated reading time.
- "Regenerate" button bypasses the local cache and forces a fresh call.
- Errors (rate-limit 429, credits 402, network) surface inline with retry.
